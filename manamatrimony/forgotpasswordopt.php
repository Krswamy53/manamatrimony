...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE  user_profiles SET otp = ? WHERE email = ?";
    if ($stmt = $conn->prepare($sql)) {
    ...
    try {
        ...
        $mail->Body = "Your OTP is: $otp<br><br><a href='http://localhost/manamatrimony/updatepassword.php?email=" . urlencode($toEmail) . "&timestamp=" . time() . "&expiry=$expiryTimestamp' style='background-color: #4CAF50; color: white; padding: 15px 25px; text-align: center; text-decoration: none; display: inline-block; border-radius: 10px;'>Change Password</a>";

        if ($mail->send()) {
            echo '<script>alert("OTP sent successfully."); window.location.href = "loginform.php";</script>';
        } else {
