...
<body>
    <div class="container-fluid">
      ...
      <a href="allprofiles.php?filter=active" class="btn btn-primary">Active (<?php echo $counts['active']; ?>)</a>
        ...
        <!-- <a href="allprofiles.php" class="btn btn-info">Pending Plans (<?php echo $counts['all']; ?>)</a> -->
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<input type='checkbox' name='selected_users[]' class='user-checkbox' value='" . $row['id'] . "' />"; // Checkbox
        ...
        // Determine button class and action
        $button_class = 'btn btn-success'; 
        $href = 'allprofiles.php'; // Default link
        ...
        if ($row['payment'] == 'unpaid') {
            ...
            $href = 'javascript:void(0);'; // Prevent default action
        }

        // Payment button
        echo "<a href='" . $href . "' class='" . $button_class . "' style='width: 150px;' 
                " . ($row['payment'] == 'pending' ? "onclick='confirmPayment(" . $row['id'] . ")'" : "") . ">
                " . ucfirst($row['payment']) . "
              </a>";
        ...
        // View More button
        echo "<a href='view.php?id=" . $row['id'] . "' class='btn btn-view btn-info ok'>View More</a>";            
...
function confirmPayment(userId) {
    if (confirm("Are you sure you want to mark this payment as Paid?")) {
        ...
        xhr.open("GET", "update_payment.php?id=" + userId, true);
        xhr.onload = function () {
            if (xhr.status == 200) {
